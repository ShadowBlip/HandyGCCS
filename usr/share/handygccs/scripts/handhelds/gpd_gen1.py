...
async def process_event(seed_event, active_keys):
    ...
    if active_keys == [29, 56, 111] and button_on == 1 and button1 not in event_queue:
        event_queue.append(button1)
        await com.emit_now(seed_event, button1, 1)
    elif active_keys == [] and seed_event.code in [29, 56, 111] and button_on == 0 and button1 in event_queue:
        event_queue.remove(button1)
        await com.emit_now(seed_event, button1, 0)

    # BUTTON 2 (Default: QAM)
    if active_keys == [1] and button_on == 1 and button2 not in event_queue:
