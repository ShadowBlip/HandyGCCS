...
async def process_event(seed_event, active_keys):
    ...
    if active_keys == [97, 100, 111] and button_on == 1 and button2 not in event_queue:
        ...
        await emit_now(seed_event, button2, 1)
        ...
        await emit_now(seed_event, button2, 0)
    ...
    # BUTTON 3 SECOND STATE (Default: Toggle Gyro)
        ...
        com.gyro_enabled = not com.gyro_enabled
        if com.gyro_enabled:
            await com.do_rumble(0, 250, 1000, 0)
        else:
            await com.do_rumble(0, 100, 1000, 0)
            await asyncio.sleep(com.FF_DELAY)
            await com.do_rumble(0, 100, 1000, 0)
