...
async def process_event(seed_event, active_keys):
    ...
    if active == [49, 125] and button_on == 1 and button3 not in event_queue:
        event_queue.append(button3)
        await handycon.emit_now(seed_event, button3, 1)
    elif active == [] and seed_event.code in [49, 125] and button_on == 0 and button3 in event_queue:
    ...
    if active == [68] and button_on == 1 and button6 not in event_queue:
        event_queue.append(button6)
        await handycon.emit_now(seed_event, button6, 1)
    elif active == [] and seed_event.code in [68] and button_on == 0 and button6 in event_queue:
    ...
    if active == [187] and button_on == 1 and button7 not in event_queue:
        event_queue.append(button7)
        await handycon.emit_now(seed_event, button7, 1)
    elif active == [] and seed_event.code in [188] and button_on == 0 and button7 in event_queue:
        event_queue.remove(button7)
        await handycon.emit_now(seed_event, button7, 0)

    # BUTTON 8 (Default: GUIDE) Ally Home Long Press.
    # This event triggers from KEYBOARD_2.
    if active == [29, 56, 111] and button_on == 1 and button8 not in event_queue:
        event_queue.append(button8)
        await handycon.emit_now(seed_event, button8, 1)
    elif active == [] and seed_event.code in [29, 56, 111] and button_on == 0 and button8 in event_queue:
        event_queue.append(button8)
        await handycon.emit_now(seed_event, button8, 0)

    # BUTTON 9 (Default:) Paddle + D-Pad DOWN
    # This event triggers from KEYBOARD_2.
    if active == [1, 29, 42] and button_on == 1 and button9 not in event_queue:
        event_queue.append(button9)
        await handycon.emit_now(seed_event, button9, 1)
    elif active == [] and seed_event.code in [1, 29, 42] and button_on == 0 and button9 in event_queue:
        event_queue.append(button9)
        await handycon.emit_now(seed_event, button9, 0)

    # BUTTON 10 (Default:) Paddle + D-Pad LEFT
    # This event triggers from KEYBOARD_2.
    if active == [32, 125] and button_on == 1 and button10 not in event_queue:
        event_queue.append(button10)
        await handycon.emit_now(seed_event, button10, 1)
    elif active == [] and seed_event.code in [32, 125] and button_on == 0 and button10 in event_queue:
        event_queue.append(button10)
        await handycon.emit_now(seed_event, button10, 0)

    # BUTTON 11 (Default:) Paddle + D-Pad RIGHT
    # This event triggers from KEYBOARD_2.
    if active == [15, 125] and button_on == 1 and button11 not in event_queue:
        event_queue.append(button11)
        await handycon.emit_now(seed_event, button11, 1)
    elif active == [] and seed_event.code in [15, 125] and button_on == 0 and button11 in event_queue:
        event_queue.append(button11)
        await handycon.emit_now(seed_event, button11, 0)

    # BUTTON 12 (Default:) Paddle + X
    # This event triggers from KEYBOARD_2.
    if active == [25, 125] and button_on == 1 and button12 not in event_queue:
        event_queue.append(button12)
        await handycon.emit_now(seed_event, button12, 1)
    elif active == [] and seed_event.code in [25, 125] and button_on == 0 and button12 in event_queue:
        event_queue.append(button12)
